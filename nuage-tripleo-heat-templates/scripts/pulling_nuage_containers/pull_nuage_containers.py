...
#####
# Generates nauge_local_regsitry_images.yaml, with Nuage conatiner
# names and push destination
#

def generate_local_regsitry_nuage_images(nuage_config):
    dirPath = os.path.dirname(os.path.abspath(__file__))
    ENV = Environment(loader=FileSystemLoader('%s/' % dirPath))
    nuage_images_file_name = 'nuage_local_regsitry_images.yaml'
    j2_nuage_images_file_name = nuage_images_file_name + '.j2'
    file_exists(j2_nuage_images_file_name)
    nuage_images_file = ENV.get_template(j2_nuage_images_file_name)
    logger.info("Rendering nuage_local_regsitry_images.yaml file")
    ...
    logger.info("Rendering nuage_local_regsitry_images.yaml is "
                "completed")
...
def generate_nuage_overcloud_images(nuage_config):
    ...
    file_exists(j2_nuage_overcloud_images_file_name)
...
# Red Hat Regsitry, retag them all with regsitry as local regsitry ,
...
def main():
    ...
    parser.add_argument("--nuage-config", dest="nuage_config",
                        ...
                        help="path to nuage_config.yaml")
    args = parser.parse_args()

    with open(args.nuage_config) as nuage_config:
        nuage_config = yaml.load(nuage_config)

    config_file = generate_local_regsitry_nuage_images(nuage_config)
